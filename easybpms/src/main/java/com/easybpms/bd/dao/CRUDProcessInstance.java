...
public class CRUDProcessInstance {
	...
	public static ProcessInstance read(ProcessInstance processInstance, EntityManager session) throws CRUDException {
		try {
			...
			/*else if(processInstance.getIdBpms() != null){
				return session.createQuery("FROM ProcessInstance WHERE idBpms = '" + processInstance.getIdBpms() + "'", ProcessInstance.class).getSingleResult();
			}
			else if(processInstance.getProcess() != null){
				return session.createQuery("FROM ProcessInstance WHERE process_id = '" + processInstance.getProcess().getId() + "'", ProcessInstance.class).getSingleResult();
			}*/
